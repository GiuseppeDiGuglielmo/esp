...
CCS_MAIN(int argc, char **argv) {
    ...
    for (unsigned t = 1; t < 10; t++) {
        ...
        for (unsigned j = 0; j < in_words; j++) {
            ac_int<WL, false> data(raw_inputs[t][j+0]);

            inputs[j] = data;

            ac_int<DMA_WIDTH, false> data_dma = data;

            dma_read_chnl.write(data_dma);
        }
        ...
        while (!dma_write_chnl.available(sha1_out_size)) {} // Testbench stalls until data ready
        for (unsigned i = 0; i < sha1_out_size; i++) {

            ac_int<DMA_WIDTH, false> data_dma = dma_write_chnl.read();

            ac_int<WL, false> data = data_dma;
